...
namespace GeometryAPI
{
    ...
    public class Cylinder : Figure, IAreaMeasurable, IVolumeMeasurable
    {
        ...
        public double GetArea()
        {
            return (Math.PI * this.Radius * this.Radius) * 2 + (2 * Math.PI * this.Radius) * (this.vertices[0] - this.vertices[1]).Magnitude;
        }

        public double GetVolume()
        {
            return (Math.PI * this.Radius * this.Radius) * (2 * Math.PI * this.Radius) * (this.vertices[0] - this.vertices[1]).Magnitude;
        }
    }
    ...
    public class ExtenedFigureController : FigureController
    {
                ...
                case "circle":
                    {
                        ...
                        currentFigure = currentFigure = new Circle(center, radius);
                        break;
                    }
                case "cylinder": // Vector3D bottom, Vector3D top, double radius
                    {
        ...
        protected override void ExecuteFigureInstanceCommand(string[] splitCommand)
        {
            switch (splitCommand[0])
            {
                case "area":
                    {
                        var areaMeasurablecurrentFigure = this.currentFigure as IAreaMeasurable;
                        if (areaMeasurablecurrentFigure != null)
                        {
                            Console.WriteLine("{0:0.00}", areaMeasurablecurrentFigure.GetArea());
                        }
                        else
                        {
                ...
                case "volume":
                    {
                        var volumeMeasurablecurrentFigure = this.currentFigure as IVolumeMeasurable;
                        if (volumeMeasurablecurrentFigure != null)
                        {
                            Console.WriteLine("{0:0.00}", volumeMeasurablecurrentFigure.GetVolume());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
                ...
                case "normal":
                    {
                        var iFlatcurrentFigure = this.currentFigure as IFlat;
                        if (iFlatcurrentFigure != null)
                        {
                            Console.WriteLine("{0:0.00}", iFlatcurrentFigure.GetNormal());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
...
namespace AcademyGeometry
{
    class Program
    {
        ...
        {
            return new ExtenedFigureController();
        }
